defmodule MilkRun.Connections.Bitfinex do
  ...
  @impl true
  def init(state) do
    Logger.debug("Initializing bitfinex connection", ansi_color: :light_blue)
  ...
  @impl Connection
  def stop do
    Logger.debug("Manually stopping bitfinex connection", ansi_color: :light_blue)
  ...
  def handle_info({:start}, state) do
    Logger.debug("Starting bitfinex connection", ansi_color: :light_blue)
  ...
  def handle_info(catch_all, state) do
    IO.puts("Bitfinex catch all below --------------")
  ...
  defp manage_connection({:ok, pid}, state) do
    Logger.debug("Bitfinex started: #{inspect(pid)}", ansi_color: :light_blue)
  ...
  defp manage_connection({:error, code, message}, state) do
    Logger.error("Bitfinex issued an error #{code} on startup : #{message}")
